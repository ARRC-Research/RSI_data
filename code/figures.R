...
dat_all%>%
  mutate(en_r = car::recode(enrtot, recodes = "0:999='1'; 1000:4999='2';5000:9999='3';10000:19999='4';20000:150000='5'", as.factor = T), 
         rsicat= factor(ifelse(sc_score<mu, "1Not RSI", ifelse(sc_score>=mu&RSI_Rescaled<mu+sds, "2RSI", "3High RSI"))))%>%
...
dat_all%>%
  mutate(en_r = car::recode(fte,
         ...
         rsicat= factor(ifelse(sc_score<mu, "1Not RSI", 
                               ifelse(sc_score>=mu&RSI_Rescaled<mu+sds, "2RSI", "3High RSI"))))%>%
  mutate(rsicat =relevel(rsicat,ref = "1Not RSI") )%>%
  #         rsi_cat = ifelse(sc_score <mu, "NonRSI", ifelse(sc_score>=mu+sds, "High RSI", "Non_RSI")))%>%
  group_by(rsicat, en_r)%>%
  summarize(ns = n())%>%
...
dat_all%>%
  ...
  mutate(sector2 = factor(ifelse(sector ==1, "Public 4 Year", "Private 4 Year")), 
         rsicat= factor(ifelse(sc_score<mu, "1Not RSI", "2RSI")))%>%
  mutate(rsicat =relevel(rsicat,ref = "1Not RSI"),
...
ggsave(filename = "fig3.png", width = 10, height=8, dpi="print")  
...
dat_all%>%
  ...
  mutate(sector2 = factor(ifelse(sector ==1, "Public 4 Year", "Private 4 Year")), 
         rsicat= factor(ifelse(sc_score<mu, "1Not RSI", 
                               ifelse(sc_score>=mu&RSI_Rescaled<mu+sds, "2RSI", "3High RSI"))))%>%
  mutate(rsicat =relevel(rsicat,ref = "2RSI"),
         sector2 = relevel(sector2, ref="Public 4 Year"))%>%
  #         rsi_cat = ifelse(sc_score <mu, "NonRSI", ifelse(sc_score>=mu+sds, "High RSI", "Non_RSI")))%>%
  filter(rsicat %in% c("3High RSI","2RSI" ))%>%
  group_by(rsicat, sector2)%>%
  summarize(efug = mean(efug, na.rm=T),
            efgrad = mean(efgrad, na.rm=T))%>%
  #mutate(pct = ns/sum(ns))%>%
...
dat_all%>%
 ...
 # mutate(name2 = relevel(name2, ref="Undergrad Enrollment"))%>%
  ...
  geom_col(aes(x=sector2,y=pgrnt_p,
           ...
           position="dodge")+
  #facet_wrap(~name2, ncol=2)+
  labs(title = "Average Pell Grant Percentage for RSIs and High RSIs by Sector",
       caption = "Source: AARC Compiled Dataset\n Calculations by Corey S. Sparks, Ph.D.",
       x = "Institution Type",
       y = "Mean")+
  #ylim(c(0, .2))+
...
dat_all%>%
  ...
  geom_col(aes(x=sector2,y=dvef15,
           ...
           position="dodge")+
  #facet_wrap(~name2, ncol=2)+
  labs(title = "Average Adult Student Percentage for RSIs and High RSIs",
       caption = "Source: AARC Compiled Dataset\n Calculations by Corey S. Sparks, Ph.D.",
       x = "Institution Type",
       y = "Mean")+
  #ylim(c(0, .2))+
...
dat_all%>%
  ...
  geom_col(aes(x=sector2,y=FTEStaff ,
           ...
           position="dodge")+
  #facet_wrap(~name2, ncol=2)+
  labs(title = "Average Full-Time Equivalent Staff for RSIs and Non-RSIs by Sector",
       caption = "Source: AARC Compiled Dataset\nCalculations by Corey S. Sparks, Ph.D.",
       x = "Institution Type",
       y = "Percentage")+
  #ylim(c(0, .2))+
...
ggsave(filename = "fig9.png", width = 10, height=8, dpi="print")  
...
dat_all%>%
  ...
  geom_col(aes(x=sector2,y=FTEStaff,
           ...
           position="dodge")+
  #facet_wrap(~name2, ncol=2)+
  labs(title = "Average Full-Time Equivalent Staff for RSIs and High RSIs by Sector",
       caption = "Source: AARC Compiled Dataset\n Calculations by Corey S. Sparks, Ph.D.",
       x = "Institution Type",
       y = "Mean")+
  #ylim(c(0, .2))+
...
ggsave(filename = "fig10.png", width = 10, height=8, dpi="print")  
...
ggsave(filename = "fig11.png", width = 10, height=8, dpi="print")  
...
dat_all%>%
  ...
  geom_col(aes(x=sector2,y=value,
           ...
           position="dodge")+
  facet_wrap(~name2, ncol=2)+
  labs(title = "Average Percent of Revenue from Student Appropriations\nand Tuition for RSIs and High RSIs",
       caption = "Source: AARC Compiled Dataset\n Calculations by Corey S. Sparks, Ph.D.",
       x = "Institution Type",
       y = "Mean")+
  #ylim(c(0, .2))+
...
ggsave(filename = "fig12.png", width = 10, height=8, dpi="print")  
...
ggsave(filename = "fig13.png", width = 10, height=8, dpi="print")  
...
ggsave(filename = "fig14.png", width = 10, height=8, dpi="print")  
...
dat_all%>%
  ...
  mutate(rsicat =relevel(rsicat,ref = "Not RSI"),
         sector2 = ordered(sector2, levels = c('Public 4 Year','Private 4 Year','Public 2 Year', 'Private 2 Year' )))%>%
  #         rsi_cat = ifelse(sc_score <mu, "NonRSI", ifelse(sc_score>=mu+sds, "High RSI", "Non_RSI")))%>%
  filter(rsicat %in% c( "Not RSI","RSI"))%>%
  group_by(rsicat, sector2)%>%
...
dat_all%>%
  ...
  4='Public 2 Year'; 5='Private 2 Year'", as.factor=T), 
         rsicat= factor(ifelse(sc_score<mu, "1Not RSI", 
                               ifelse(sc_score>=mu&RSI_Rescaled<mu+sds, "RSI", "High RSI"))))%>%
